...
public class HopperShootCommand extends Command {
	...
	protected void execute() {
		...
		m.pushProfile(TrapezoidalProfile.getTrapezoidZero(124.0 / HardwareConstants.ROTATION_CALC_FACTOR, 140, 3*Math.PI/2, 0), true, false);

		// Go left or right by 64 inches (really just 43 but it hits the wall to align)
		if (alliance == alliances.RED) {
			m.pushProfile(TrapezoidalProfile.getTrapezoidZero(64.0 / HardwareConstants.ROTATION_CALC_FACTOR, 140, 0, 0), false, true);
...
//			m.waitUntilProfileFinishes(100);
			...
			m.pushProfile(TrapezoidalProfile.getTrapezoidZero(6.0 / HardwareConstants.ROTATION_CALC_FACTOR, 90, 0, 0), false, true);
			m.pushTurn(0.172, false, true);
			m.waitUntilProfileFinishes(100);
		}
